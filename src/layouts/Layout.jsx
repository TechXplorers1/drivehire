...
export default function Layout() {
    ...
    const isAccountPage = ['/account', '/driver', '/admin'].some(path => location.pathname.startsWith(path));

    return (
        <div className="min-h-screen bg-white flex flex-col">
            {!isAccountPage && <Navbar />}
            <main className={`${isAccountPage ? '' : (isTransparentPage ? '' : 'pt-20')} mx-auto flex-grow w-full`}>
                <Outlet />
            </main>
            {!isAccountPage && (
